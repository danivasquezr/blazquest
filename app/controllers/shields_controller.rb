class ShieldsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @shield.save
        ...
        format.json { render action: 'show', status: :created, location: @shield }
      else
  ...
  def destroy
    @shield.destroy
    respond_to do |format|
      format.html { redirect_to shields_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def shield_params
      params.require(:shield).permit(:shield, :origin, :description, :surname_id)
    end
end
